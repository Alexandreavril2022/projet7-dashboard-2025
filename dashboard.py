...
def main():
    ...
    API_URL = "https://projet7-loan-api-2025-43568ca8afe1.herokuapp.com/app"
    # -----------------------------------------------
    ...
    def get_list_display_features(f, def_n, key):
        ...
        n = st.slider(
            ...
            min_value=2, max_value=40, value=def_n,
            key=f"{key}_slider"
        )
    ...
    def get_score_model(selected_id):
        # URL of the sk_id API
        score_api_url = f"{API_URL}/scoring_cust?SK_ID_CURR={str(selected_id)}"
        print("AAAIHJEFI8HFIUZEHFZEUHF",score_api_url)
        # Requesting the API and saving the response
    ...
    def get_data_fivehundred_neigh(selected_id):
        fivehundred_neight_data_api_url = f"{API_URL}/thousand_neigh/?SK_ID_CURR={str(selected_id)}"
        # save the response of API request
        response = requests.get(fivehundred_neight_data_api_url)
        # convert from JSON format to Python dict
        content = json.loads(response.content.decode('utf-8'))
        # convert data to pd.DataFrame and pd.Series
        data_fivehundred_neig = pd.DataFrame(content['X_thousand_neigh'])
        x_custo = pd.DataFrame(content['x_custom'])
        target_fivehundred_neig = (pd.Series(content['y_thousand_neigh']).rename('TARGET'))
